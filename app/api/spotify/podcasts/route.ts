...
export async function GET(request: NextRequest) {
  try {
    console.log('API: Fetching user podcast episodes...');
    ...
    console.log(`API: Getting user podcast episodes with limit=${limit}`);

    // Make the API call to get user's saved episodes
    ...
    if (!response.ok) {
      ...
      // If it's a permissions error, return empty episodes instead of throwing
      if (response.status === 403) {
        console.warn('⚠️ Insufficient permissions to access podcast data. Returning empty array.');
    ...
    // Transform the data to extract the episode information
    const episodes = data.items.map((item: any) => item.episode);

    console.log(`API: Successfully retrieved ${episodes.length} podcast episodes`);
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to fetch podcast episodes' },
      { status: statusCode }
    );
  }
}
